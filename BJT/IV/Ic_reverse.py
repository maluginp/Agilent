...
def execute():
    ...
    try:
        ...
        if  ag.get_good_range( "v", Measurements["Ue"] ) != ag.get_range(1,"v"):
            rng = ag.get_good_range( "v", Measurements["Ue"] )
            ag.set_range(1,"v",rng)
            print "@CH1 New range: %s , V = %.2f" % (rng, Measurements["Ue"] )
            re_measure()
...
if INCLUDE_GRAPHICS:
    plt.title("I-V output. Reverse mode")
    plt.xlabel("-Uc, V")
    plt.ylabel("-Ic, A")
    plt.grid(True)
