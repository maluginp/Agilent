...
# Ube
_Ube = config.get('Agilent','Ube')
Ube_RANGES = ag.get_source_range(_Ube)
# Uce
...
fmeasure = open(out_dir + "tsunami_in.txt", "w")
...
def re_measure(cycle=1):
    ...
    Ube = Ube_m = Ib = Uce = Uce_m = Ic = Temp = 0.0
    ...
    for i in xrange(cycle):
        ...
        Ib    += ag.measure(CHANNEL_UBE,'c')
        Ube   += ag.source_value(CHANNEL_UBE,'v')
        Ube_m += ag.measure(CHANNEL_UBE,'v')
    ...
    Measurements['Ube_m'] = Ube_m / float(cycle)
...
def execute():
    ...
    # _log("  \tUbe\tUbe_m\tIb        \tUce\tUce_m\tIc\t        Temp")
    buf = "%s\t%.2f\t%.2f\t%.2e\t%.2f\t%.2f\t%.2e\t%.2f" % (_smile, 
        meas('Ube'),meas('Ube_m'),meas('Ib'),meas('Uce'), \
...
for Uce in Uce_RANGES:
    ...
    for Ube in Ube_RANGES:
        ag.source(CHANNEL_UBE,'v',Ube)
        if not execute():
